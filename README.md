...
## Multi-Harness Orchestrator CLI

This project includes a custom **Multi-Harness Orchestrator** that lets you call **agents**, **skills**, **commands**, and **plugins** simultaneously across all supported harnesses.
...
marketplace ls
marketplace ls --plugins --plugin python
marketplace ls --agents -a
marketplace ls --skills -s
marketplace ls --commands -c

# Invoke a single agent across ALL harnesses simultaneously
marketplace invoke --agent python-development__python-pro

# Invoke a skill across selected harnesses
marketplace invoke --skill python-development__python-testing-patterns \
                   --harness source --harness claude --harness codex

# Invoke multiple targets at once (plugin + agent + skill)
marketplace invoke --plugin python-development \
                   --agent python-development__python-pro \
                   --skill python-development__python-testing-patterns

# Output results as JSON
marketplace invoke --agent python-development__python-pro --json

# Regenerate harness artifacts
marketplace sync                  # all harnesses
marketplace sync --harness codex  # single harness
```

### Supported Harnesses

| Harness | Adapter | Artifacts |
|---|---|---|
| **Source** | `SourceHarness` | Reads directly from `plugins/` |
| **Claude Code** | `ClaudeHarness` | `/agent`, `/skill`, `/command` actions |
| **Codex CLI** | `CodexHarness` | `.codex/agents/`, `.codex/skills/` |
| **Cursor** | `CursorHarness` | `.cursor-plugin/`, `.cursor/rules/` |
| **OpenCode** | `OpenCodeHarness` | `.opencode/{agents,commands,skills}/` |
| **Gemini CLI** | `GeminiHarness` | `agents/`, `skills/`, `commands/` |
| **Copilot** | `CopilotHarness` | `.copilot/{agents,skills,commands}/` |
...
orchestrator = MultiHarnessOrchestrator(
    registry,
    harnesses=["source", "claude", "codex", "opencode", "gemini", "cursor", "copilot"]
)

# Call everything simultaneously
